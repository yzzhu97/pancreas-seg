...
for i in range(total_samples):
    ...
    for plane in ['Z']:
        ...
        sum_ = np.zeros((slice_number, organ_number + 1), dtype = np.int32)
        # for storing bounding boxes of ground truth masks (A_min, A_max, B_min, B_max)
        minA = np.zeros((slice_number, organ_number + 1), dtype = np.int32)
        maxA = np.zeros((slice_number, organ_number + 1), dtype = np.int32)
        minB = np.zeros((slice_number, organ_number + 1), dtype = np.int32)
        maxB = np.zeros((slice_number, organ_number + 1), dtype = np.int32)
        # for storing mean pixel value of each slice
        average = np.zeros((slice_number), dtype = np.float32)
